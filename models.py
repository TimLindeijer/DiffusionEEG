...
if __name__ == "main":
    ...
    # Instantiate modules
    mlp = Mlp(in_features=emb_size, hidden_features=256, dtype=dtype)
    generic_tfb = GenericTFB(emb_size=emb_size, num_heads=num_heads, dtype=dtype)
    temporal_tfb = TemporalTFB(emb_size=emb_size, num_heads=num_heads, avgf=5, dtype=dtype)
    odcm = ODCM(input_channels=input_channels, kernel_size=kernel_size, dtype=dtype)
    rtm = RTM(input=torch.randn(10, 20, emb_size), num_blocks=num_blocks, num_heads=num_heads, dtype=dtype)
    stm = STM(input=torch.randn(10, 20, emb_size), num_blocks=num_blocks, num_heads=num_heads, dtype=dtype)
    ttm = TTM(input=torch.randn(3, 10, emb_size), num_submatrices=2, num_blocks=num_blocks, num_heads=num_heads, dtype=dtype)

    #Test
    x = torch.randn(10, emb_size)  # Batch size 10
    output = mlp(x)
    print("MLP Output Shape:", output.shape)

    x = torch.randn(10, 20, emb_size)  # Batch size 10, seq length 20
    savespace = torch.randn(20, 10, emb_size)
    output = generic_tfb(x, savespace)
    print("GenericTFB Output Shape:", output.shape)

    x = torch.randn(10, 20, emb_size)  # Batch size 10, seq length 20
    savespace = torch.randn(5, emb_size)
    output = temporal_tfb(x, savespace)
    print("TemporalTFB Output Shape:", output.shape)

    x = torch.randn(input_channels, 50)  # Input channels and sequence length
    output = odcm(x)
    print("ODCM Output Shape:", output.shape)

    x = torch.randn(10, 20, emb_size)
    output = rtm(x)
    print("RTM Output Shape:", output.shape)

    x = torch.randn(10, 20, emb_size)
    output = stm(x)
    print("STM Output Shape:", output.shape)

    x = torch.randn(3, 10, emb_size)
    output = ttm(x)
    print("TTM Output Shape:", output.shape)
