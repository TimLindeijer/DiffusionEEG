...
def adjust_learning_rate(optimizer, epoch, args):
    """
    Adjusts the learning rate of an optimizer based on the current epoch.
    ...
    The function provides multiple schedule options with more gradual decay patterns.
    """
    # Default case - no adjustment
    lr = args.learning_rate
    
    if args.lradj == 'type1':
        # Modified type1: Slower decay using 0.8 instead of 0.5
        # and decay interval of 2 epochs instead of 1
        lr_adjust = {epoch: args.learning_rate * (0.8 ** ((epoch - 1) // 2))}
    
    elif args.lradj == 'type2':
        # Modified type2: More gradual steps with smaller decreases
        lr_adjust = {
            2: args.learning_rate * 0.7,    # Was 5e-5
            4: args.learning_rate * 0.5,    # Was 1e-5
            6: args.learning_rate * 0.3,    # Was 5e-6
            8: args.learning_rate * 0.2,    # Was 1e-6
            10: args.learning_rate * 0.1,   # Was 5e-7
            15: args.learning_rate * 0.05,  # Was 1e-7
            20: args.learning_rate * 0.02   # Was 5e-8
        }
    
    elif args.lradj == "cosine":
        # Modified cosine: Slower decay with a less aggressive scale factor
        # Multiplying by 0.8 instead of 0.5 for a more gentle decrease
        lr_adjust = {epoch: args.learning_rate * 0.8 * (1 + math.cos(epoch / args.train_epochs * math.pi))}
    
    elif args.lradj == "linear":
        # New option: Simple linear decay
        decay_factor = max(0.1, 1 - (epoch / args.train_epochs) * 0.9)
        lr_adjust = {epoch: args.learning_rate * decay_factor}
    
    elif args.lradj == "step":
        # New option: Step decay with configurable parameters
        step_size = getattr(args, 'lr_step_size', 5)
        gamma = getattr(args, 'lr_gamma', 0.9)  # 0.9 means 10% reduction
        lr_adjust = {epoch: args.learning_rate * (gamma ** (epoch // step_size))}
    
    elif args.lradj == "warmup_cosine":
        # New option: Warmup followed by cosine decay
        warmup_epochs = getattr(args, 'warmup_epochs', 5)
        if epoch <= warmup_epochs:
            # Linear warmup
            lr_adjust = {epoch: args.learning_rate * (epoch / warmup_epochs)}
        else:
            # Cosine decay after warmup (more gradual than before)
            progress = (epoch - warmup_epochs) / (args.train_epochs - warmup_epochs)
            lr_adjust = {epoch: args.learning_rate * 0.8 * (1 + math.cos(progress * math.pi))}
    
    # Apply the learning rate if it's defined for this epoch
    if hasattr(args, 'lradj') and args.lradj and epoch in lr_adjust.keys():
        lr = lr_adjust[epoch]
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        print(f'Updating learning rate to {lr:.6e}')
