...
def run_mne_cluster_permutation_test(real_psds, synthetic_psds, freqs, n_permutations=1000, alpha=0.05):
    ...
    print(f"Testing {n_channels} channels × {n_freqs} frequencies = {n_channels * n_freqs} features")
    ...
    if len(clusters) > 0:
        print(f"First cluster type: {type(clusters[0])}, size: {len(clusters[0]) if hasattr(clusters[0], '__len__') else 'N/A'}")
    ...
    if len(clusters) > 0:
        for cluster_idx, (cluster, p_val) in enumerate(zip(clusters, cluster_p_values)):
            if p_val < alpha:
                # cluster contains indices of significant features
                # Create a flat mask and set cluster indices to True
                flat_mask = np.zeros(n_channels * n_freqs, dtype=bool)
                flat_mask[cluster] = True
                
                # Reshape to 2D
                cluster_2d = flat_mask.reshape(n_channels, n_freqs)
...
if __name__ == "__main__":
    ...
    results = run_group_psd_analysis(
        ...
        output_dir='images/group_statistical_analysis_SYNTH-CAUEEG2-NORMALIZED',
        n_permutations=1000
    )
