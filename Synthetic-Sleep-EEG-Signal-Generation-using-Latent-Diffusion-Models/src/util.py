...
def setup_run_dir(config, args, base_path):
    ...
    print(f"Run dir: {config.train.run_dir}")
    print(f"Spe: {args.spe}")
    print(f"Dataset: {args.dataset}")
    # run_dir = output_dir / str(config.train.run_dir+"_"+args.spe+"_"+args.dataset)
    run_dir = output_dir / str(config.train.run_dir)
    ...
    if torch.cuda.is_available():
        ...
        for i in range(torch.cuda.device_count()):
            ...
            if memory_reserved > 0:
                ...
                utilization = 0
            print(f"gpu:{i} allocated: {memory_allocated/1024**2:.2f}MB, reserved: {memory_reserved/1024**2:.2f}MB, util: {utilization:.2f}%")
