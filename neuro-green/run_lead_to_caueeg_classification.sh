...
#SBATCH --job-name=mlr_diffusion_comparisons
#SBATCH --output=outputs/mlr_diffusion_comparisons_%j.out
...
OUTPUT_DIR="results/TRANSFER_SYNTH-CAUEEG2-NORMALIZED_${COMBINATION}"
mkdir -p $OUTPUT_DIR

# Create run name with combination and timestamp
RUN_NAME="TRANSFER_SYNTH-CAUEEG2-NORMALIZED_${COMBINATION}_$(date +%Y%m%d_%H%M%S)"
