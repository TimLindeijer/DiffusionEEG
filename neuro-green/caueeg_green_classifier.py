...
def load_caueeg_data(data_dir, derivatives_subdir):
    ...
    # List all available folders in derivatives
    available_folders = set([f for f in os.listdir(derivatives_path) if f.startswith('sub-')])
    print(f"Found {len(available_folders)} folders in derivatives directory")
    ...
    for _, row in participants_df.iterrows():
        ...
        eeg_file_patterns = [
            os.path.join(
                ...
                participant_id,
                'eeg',
                f'{participant_id}_task-eyesClosed_desc-reject[]_eeg.fif'
            ),
            os.path.join(
                derivatives_path,
                participant_id,
                'eeg',
                f'{participant_id}_task-eyesClosed_desc-reject_eeg.fif'
            )
        ]
